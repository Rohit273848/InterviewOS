  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] flex items-center justify-center p-4 md:p-8 font-sans text-white selection:bg-[#B1F82A]/30 selection:text-[#B1F82A] relative overflow-hidden">
      ...
      <div
        ...
        aria-hidden="true"
      />
      <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-[#B1F82A]/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="w-full max-w-[1200px] h-auto lg:h-[720px] bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[32px] shadow-2xl flex flex-col lg:flex-row overflow-hidden relative z-10">
        ...
        <div className="w-full lg:w-[42%] flex flex-col p-8 md:p-[60px] justify-center relative">
          ...
          <div className="max-w-[400px] w-full mx-auto mt-12 lg:mt-0">
            <div className="mb-10">
              <h1 className="text-[32px] font-bold text-white tracking-tight mb-2">Create an account</h1>
              <p className="text-gray-400 text-[15px]">Join InterviewOS to accelerate your placement prep.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <div className="relative group">
                  <User className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-500 group-focus-within:text-[#B1F82A] transition-colors" />
                  <input
                    type="text"
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full h-[52px] bg-[#B1F82A] text-black font-bold rounded-full hover:scale-[1.02] hover:bg-[#9DE024] hover:shadow-[0_0_20px_rgba(177,248,42,0.3)] transition-all active:scale-[0.98] disabled:opacity-70 disabled:hover:scale-100 flex items-center justify-center mt-4"
              >
                {isLoading ? (
            ...
            <div className="mt-8 mb-8 flex items-center justify-center gap-4">
              ...
              <span className="text-[13px] text-gray-500 font-medium">or register with</span>
              <div className="flex-1 h-px bg-white/10"></div>
            </div>

            <div className="flex justify-center gap-4">
              <button onClick={() => handleSocialSignup('Google')} className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 hover:border-[#B1F82A]/30 transition-all group">
                <GoogleIcon className="w-5 h-5 text-gray-300 group-hover:text-white transition-colors" />
              </button>
              <button onClick={() => handleSocialSignup('GitHub')} className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 hover:border-[#B1F82A]/30 transition-all group">
                <Github className="w-5 h-5 text-gray-300 group-hover:text-white transition-colors" />
              </button>
              <button onClick={() => handleSocialSignup('LinkedIn')} className="w-12 h-12 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 hover:border-[#B1F82A]/30 transition-all group">
            ...
            <p className="text-center text-[14px] text-gray-400 mt-10">
              ...
              <Link to="/signin" className="text-[#B1F82A] font-semibold hover:underline">
                Sign in
              </Link>
            </p>
          </div>
